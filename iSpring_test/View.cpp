...
void Show()
{
	...
	while (window.isOpen())
	{
		...
		if (event.type == Event::MouseButtonPressed && event.key.code == Mouse::Left) // добавить условие единичного нажати€
		{
			if (a == 0)
			{
				Menu_Click_Check();
		...
		if (event.type == Event::MouseButtonReleased && event.key.code == Mouse::Left)
			a = 0;
		if (event.type == Event::KeyPressed && event.key.code == Keyboard::Delete) // добавить условие единичного нажати€
		{
			if (b == 0)
			{
